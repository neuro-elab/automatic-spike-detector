...
def apply_line_length(eeg_data: np.array, sfreq: int):
    ...
    time_points = np.round(np.linspace(0, duration - 1, max(2, round(duration / 40000)))).astype(dtype=int)
    ...
    for idx in range(len(time_points) - 1):
        ...
        if idx == len(time_points) - 2:
            eeg_interval = np.concatenate(
                (eeg_data[:, time_points[idx]: time_points[idx + 1]], np.zeros((nr_channels, w_eff))), axis=1
            )
        else:
